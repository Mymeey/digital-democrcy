...
interface AppStore {
  ...
  joinOrganization: (code: string, realName: string) => Promise<boolean>;
...
export const useStore = create<AppStore>((set, get) => ({
  ...
  // 組織コードで参加
  joinOrganization: async (code: string, realName: string): Promise<boolean> => {
    const { user, organizations } = get();
    ...
    // App Review向け: REVIEW01 は組織メンバーとして参加可能にする
    if (!targetOrg && code.toUpperCase() === REVIEW_ORG_CODE) {
      targetOrg = {
        id: REVIEW_ORG_ID,
        name: '審査用組織',
